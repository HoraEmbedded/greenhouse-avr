...
so the effect of the hysteresis logic is visible directly.
...
def build_figure(timestamps, temps, soils, fan, pump):
    """Builds and returns the figure, separated from file I/O so it can be
    exercised by a test without writing a PNG to disk."""
...
def main() -> None:
    ...
    timestamps, temps, soils, fan, pump = load_csv(args.input)
    if not timestamps:
        raise SystemExit(f"No rows found in {args.input}")

    fig = build_figure(timestamps, temps, soils, fan, pump)
